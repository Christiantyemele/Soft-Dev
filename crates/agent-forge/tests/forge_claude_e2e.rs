...
async fn test_forge_dangerous_command_suspends() -> Result<()> {
    ...
    // Make it executable
    std::process::Command::new("chmod").arg("+x").arg(&mock_claude_path).spawn()?.wait()?;
    ...
    // 4. Verify Store
    let final_slots: HashMap<String, WorkerSlot> = store.get_typed("worker_slots").await.ok_or_else(|| anyhow::anyhow!("No worker_slots in store"))?;
