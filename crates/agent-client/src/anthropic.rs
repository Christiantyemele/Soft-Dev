    ...
    for msg in messages {
        match msg {
            ...
            Message::Assistant { content } => {
                let blocks: Vec<Value> = content.iter().map(|b| match b {
...
impl LlmClient for AnthropicClient {
    ...
    async fn send(
        ...
        let raw:  Value = resp.json().await.context("Failed to parse Anthropic response")?;

        if !status.is_success() {
            bail!("Anthropic API error {}: {}", status, raw["error"]["message"].as_str().unwrap_or("unknown"));
        }

        debug!(stop_reason = raw["stop_reason"].as_str(), "← Anthropic response");
